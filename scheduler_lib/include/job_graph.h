...
private:
    static auto from_type_func()
    {
        auto && [ from, to ] = std::declval<VertexF>()(std::declval<EdgeT>());
        unused_variable(to);
        return from;
    }
    static auto to_type_func()
    {
        auto && [ from, to ] = std::declval<VertexF>()(std::declval<EdgeT>());
...
template <typename EdgeT, typename VertexF> class graph : private VertexF {
public:
...
private:
    ...
    template <typename Iterator> void init_edges(Iterator begin, Iterator end)
    {
        std::for_each(begin, end, [this](const auto& edge) {
            const auto & [ from, to ] = vertex_function()(edge);
    ...
    {
        auto it
            = std::lower_bound(m_vertices.begin(), m_vertices.end(), vertex);
